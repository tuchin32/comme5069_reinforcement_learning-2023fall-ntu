...
def test_correctness(filename="tasks/maze.txt"):
    ...
    for _, a, r, d, t, ns in zip(state, action, reward, done, truncated, next_state):
        ...
        if done_prediction:
            ...
            tmp = (next_state_prediction in grid_world._init_states) and reward_prediction == r and done_prediction == d and truncated_prediction == t
            if not tmp:
                print(f"Prediction:\t{next_state_prediction},\t{reward_prediction},\t{done_prediction},\t{truncated_prediction}")
                print(f"Ground truth:\t{ns},\t{r},\t{d},\t{t}")
            ...
            tmp = next_state_prediction == ns and reward_prediction == r and done_prediction == d and truncated_prediction == t
            if not tmp:
                print(f"Prediction:\t{next_state_prediction},\t{reward_prediction},\t{done_prediction},\t{truncated_prediction}")
                print(f"Ground truth:\t{ns},\t{r},\t{d},\t{t}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", "-t", type=str, default="tasks/lava.txt")
    args = parser.parse_args()

    if "door" in args.task:
        algorithm = A2C
        print("Using A2C for door task")
    else:
        algorithm = PPO
        print("Using PPO for other tasks")
    
    # import ipdb; ipdb.set_trace()

    test_task(args.task, algorithm=algorithm)
    test_speed(args.task)
    test_correctness(args.task)
