...
class GridWorld:
    ...
    def step(self, action: int) -> tuple:
        ...
        #   - Goal, Trap, Exit: a terminal state that ends the episode with any action
        ...
        # Check whether the next state is a terminal state
        #   - Lavas: a terminal state that immediately ends the episode
        ...
        if self._is_baited:
            self.step_reward = self._bait_step_penalty
        else:
            self.step_reward = 0

        # Check the next state and update the reward
        if self._is_key_state(next_state_coord):
            self.open_door()
            self.step_reward += self._step_reward
        elif self._is_bait_state(next_state_coord):
            self.bite()
            self.step_reward = self._bait_reward
        else:
            self.step_reward += self._step_reward
    ...
    def reset(self) -> int:
        ...
        self.close_door()
        self.place_bait()
        current_index = np.random.randint(len(self._init_states))
        self._current_state = self._init_states[current_index]
        return self._current_state
