...
class MonteCarloPolicyIteration(DynamicProgramming):
    ...
    def policy_evaluation(self, state_trace, action_trace, reward_trace) -> list:
        ...
        for i in reversed(range(len(state_trace))):
            ...
            # PLOT #
            est_loss_trace.append(np.abs(G - self.q_values[s, a]))
        
        return est_loss_trace
    ...
    def run(self, max_episode=1000) -> None:
        ...
        while iter_episode < max_episode:
            ...
            if done:
                est_loss_trace = self.policy_evaluation(state_trace, action_trace, reward_trace)
                self.policy_improvement()

                # import ipdb; ipdb.set_trace()

                # PLOT #
                self.episode_reward.append(np.mean(reward_trace))
                self.episode_abs_est_loss.append(np.mean(est_loss_trace))
...
class SARSA(DynamicProgramming):
    ...
    def policy_eval_improve(self, s, a, r, s2, a2, is_done):
    ...
    def run(self, max_episode=1000) -> None:
        ...
        while iter_episode < max_episode:
            ...
            while not is_done:
                ...
                abs_td_error = self.policy_eval_improve(prev_s, prev_a, prev_r, next_s, next_a, is_done)
                
                prev_s = next_s
                prev_a = next_a

                reward_trace.append(prev_r)
                est_loss_trace.append(abs_td_error)
...
class Q_Learning(DynamicProgramming):
    ...
    def only_for_estimation_loss(self, s, a, r, s2, is_done):
    ...
    def run(self, max_episode=1000) -> None:
        ...
        while iter_episode < max_episode:
            ...
            while not is_done:
                ...
                # PLOT #
                reward_trace.append(prev_r)
                abs_td_error = self.only_for_estimation_loss(prev_s, prev_a, prev_r, next_s, is_done)
                est_loss_trace.append(abs_td_error)
