...
def run_policy_evaluation(grid_world: GridWorld):
    ...
    grid_world.visualize(
        ...
        show=False,
        filename=f"{OUTPUT_PATH}/iterative_policy_evaluation.png",
    )
...
def run_policy_iteration(grid_world: GridWorld):
    ...
    grid_world.visualize(
        ...
        show=False,
        filename=f"{OUTPUT_PATH}/policy_iteration.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(policy_iteration.policy, START_STATE)
    print(
...
def run_value_iteration(grid_world: GridWorld):
    ...
    grid_world.visualize(
        ...
        show=False,
        filename=f"{OUTPUT_PATH}/value_iteration.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(value_iteration.policy, START_STATE)
    print(
        f"Start state: {bold(green(history[0][0]))}, End state: {bold(red(history[-1][0]))}"
    )
    grid_world.reset()
...
def run_async_dynamic_programming(grid_world: GridWorld):
    ...
    grid_world.visualize(
        ...
        show=False,
        filename=f"{OUTPUT_PATH}/async_dynamic_programming.png",
    )
    print(f"Solved in {bold(green(grid_world.get_step_count()))} steps")
    history = grid_world.run_policy(async_dynamic_programming.policy, START_STATE)
    print(
        f"Start state: {bold(green(history[0][0]))}, End state: {bold(red(history[-1][0]))}"
    )
    grid_world.reset()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--maze', '-mz', type=str, default='maze.txt')
    parser.add_argument('--step_reward', '-sr', type=float, default=-1.0)
    parser.add_argument('--goal_reward', '-gr', type=float, default=1.0)
    parser.add_argument('--trap_reward', '-tr', type=float, default=-1.0)
    parser.add_argument('--discount_factor', '-df', type=float, default=0.9)
    parser.add_argument('--start_state', '-ss', type=int, default=0)
    ...
    grid_world = init_grid_world(args.maze)
